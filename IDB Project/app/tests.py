...
def insert_dummy_family(my_family):
    ins = my_family.insert().values(family_id=3, family_name="", cards_in_family=1,
                                    types_in_family=1, avg_attack=1, avg_defence=1)
    conn.execute(ins)


def insert_dummy_cardType(my_cardType):
    ins = my_cardType.insert().values(cardType_id=2, cardType_name="",
                                      cards_in_cardType=1, url="", number_of_subtypes=1)
    conn.execute(ins)


def insert_dummy_subtype(my_subType):
    ins = my_subType.insert().values(subType_id=5, cardType_id=1, subType_name="",
                                     cards_in_subType=1, avg_price_subtype=1, cardType="")
    conn.execute(ins)
...
class TestModels(TestCase):
    ...
    def test_my_cards_table(self):
        ...
        self.assertEquals(
            [(1, 1, 2, 3, "blak", "", "", "", "", 1, 1, 1, 1.0, "")], list(result2))
    ...
    def test_my_cards_table_3(self):
        ...
        # testcase 3
        ins = my_cards.insert().values(subType_id=2, cardType_id=3,
                                       name="", text="", cardType="", subType="")
        conn.execute(ins)
        result2 = conn.execute(select([my_cards]))
    ...
    def test_my_subtype(self):
        ...
        self.assertEquals(
            [(1, 3, "", "", 1.9, ""), (5, 1, '', 1, 1.0, '')], list(result2))
    ...
    def test_my_family(self):
        ...
        self.assertEquals([(1, "", "", "", 1000.0, 1100.0),
                           (3, '', 1, 1, 1.0, 1.0)], list(result2))
    ...
    def test_my_family_3(self):
        ...
        self.assertEquals([(1, "light", 1504, 42, 2401.1, 1942.3),
                           (3, '', 1, 1, 1.0, 1.0)], list(result2))
    ...
    def test_my_cardType(self):
        ...
        # testcase 1
        ins = my_cardType.insert().values(cardType_id=1, cardType_name="",
                                          cards_in_cardType="", url="", number_of_subtypes="")
        conn.execute(ins)
        result2 = conn.execute(select([my_cardType]))

        self.assertEquals(
            [(1, "", "", "", ""), (2, '', 1, '', 1)], list(result2))
    ...
    def test_my_cardType_3(self):
        ...
        # testcase 3
        ins = my_cardType.insert().values(cardType_id=1, cardType_name="Monster",
                                          cards_in_cardType=4556, number_of_subtypes=36)
        conn.execute(ins)
        result2 = conn.execute(select([my_cardType]))

        self.assertEquals([(1, "Monster", 4556, None, 36),
                           (2, '', 1, '', 1)], list(result2))
